...
\begin{enumerate}
	...
	% If the empty state is in the language then the initial state must be accepting,
	% otherwie rejecting
	\\
	Regular Expression: $b^*(ab*ab*ab*)^*b^*$
	\\
	State Diagram:\\
	 \begin{tikzpicture}
        ...
        (q3) edge[left] node{a} (q1)
        (q3) edge[loop below] node{b} (q3)
	  	;
	 \end{tikzpicture}
	
    \item \textbf{The language consisting of all words that contain "babb".}
    % The shortest word in the lanuage must have a direct path to an accepting state
    \\
    ...
    \\
    % to get the negation / complement of  lenguage, swap the accepting/rejecting states
    ...
    \begin{tikzpicture}
    	...
    	(q1) edge[bend  right, above] node{b} (q3)
    	(q2) edge[bend  left, above] node{a} (q4)
    	(q3) edge[bend  right, above] node{a,b} (q4)
    	(q4) edge[bend  left, below] node{a,b} (q1)
    	;
    \end{tikzpicture}
...
\begin{tikzpicture}
	...
	% from 15
	(q15) edge[bend  left, below] node{10} (q25)
...
% Any DFA has a regex, any regex has a DFA
% For writing out regex, since the language is finit, just OR together all possible words in the language
\\\\
