\input{header.tex}

\begin{enumerate}

\item \textbf{Concisely describe (in English) the languages accepted by the following DFAs:}

\begin{enumerate}
    \item Words ending in two or more $a$'s or $b$'s

  \begin{tikzpicture}
      \node[state, initial] (q1) {$q_1$};
      \node[state, above right of=q1] (q2) {$q_2$};
      \node[state, below right of=q1] (q5) {$q_5$};
      \node[state, accepting, right of=q2] (q3) {$q_3$};
      \node[state, accepting, right of=q5] (q4) {$q_4$};
      \draw
      (q1) edge[above] node{a} (q2)
      (q1) edge[above] node{b} (q5)

      (q2) edge[bend right=10,  left] node{b} (q5)
      (q5) edge[bend right=10, right] node{a} (q2)

      (q2) edge[above] node{a} (q3)
      (q5) edge[above] node{b} (q4)

      (q3) edge[left] node{b} (q5)
      (q4) edge[right] node{a} (q2)

      (q3) edge[loop above] node{a} (q3)
      (q4) edge[loop above] node{b} (q4)
      ;
  \end{tikzpicture}
    \item Odd number of $aaa*$

  \begin{tikzpicture}
      \node[state, initial] (q1) {$q_1$};
      \node[state, right of=q1] (q2) {$q_2$};
      \node[state, accepting, right of=q2] (q3) {$q_3$};
      \node[state, below of=q1] (q4) {$q_4$};
      \node[state, right of=q4] (q5) {$q_5$};
      \node[state, right of=q5] (q6) {$q_6$};
      \draw
      (q1) edge[loop above] node{b} (q1)
      (q1) edge[bend left=10] node{a} (q2)
      (q2) edge[bend left=10] node{b} (q1)
      (q2) edge[above] node{a} (q3)
      (q3) edge[loop above] node{a} (q3)
      (q3) edge[right] node{b} (q6)
      (q6) edge[bend left=10] node{a} (q5)
      (q5) edge[bend left=10] node{b} (q6)
      (q5) edge[above] node{a} (q4)
      (q6) edge[loop right] node{b} (q6)
      (q4) edge[loop left] node{a} (q4)
      (q4) edge[left] node{b} (q1)
      ;
  \end{tikzpicture}
\end{enumerate}

\newpage
\item \textbf{Minimize the following DFAs:}

\begin{enumerate}
    \item

  \begin{tikzpicture}
      \node[state, initial] (q1) {$q_1$};
      \node[state, above right of=q1] (q2) {$q_2$};
      \node[state, below right of=q1] (q3) {$q_3$};
      \node[state, accepting, right of=q2] (q4) {$q_4$};
      \node[state, right of=q3] (q5) {$q_5$};
      \draw
      (q1) edge[bend left=10] node{a} (q2)
      (q2) edge[bend left=10] node{a} (q1)
      (q1) edge[bend left=10] node{b} (q3)
      (q3) edge[bend left=10] node{b} (q1)
      (q3) edge[bend right=10] node{a} (q2)
      
      (q2) edge[above] node{b} (q4)
      (q5) edge[below] node{a} (q3)
      
      (q4) edge[bend left=10] node{a,b} (q5)
      (q5) edge[bend left=10] node{b} (q4)
      
      ;
  \end{tikzpicture}

Minimizes to:
\begin{table}[]
	\begin{tabular}{|c|c|c|c|c|c}
		\hline
		&  5 &  4 & 3  &   2 \\ \hline
		1 & X & X & $\checkmark$ & X\\ \hline
		2  & $\checkmark$ & X & X \\ \hline
		3  & X &  X\\ \hline
		4  &  X \\ \hline
	\end{tabular}
\end{table}


\begin{tikzpicture}
	\node[state, initial] (q1) {$q_{1,3}$};
	\node[state, right of=q1] (q2) {$q_{2,5}$};
	\node[state, accepting, right of=q2] (q4) {$q_4$};
	\draw
    (q1) edge[loop above] node{b} (q1)
	(q1) edge[bend left=10] node{a} (q2)
	(q2) edge[bend left=10] node{a} (q1)
	
	(q2) edge[above] node{b} (q4)
    (q4) edge[bend left, above] node{a,b} (q2)
	
	;
\end{tikzpicture}


    \item

  \begin{tikzpicture}
    \node[state, initial] (q1) {$q_1$};
	\node[state, below of=q1] (q2) {$q_2$};
	\node[state, accepting, right of=q1] (q3) {$q_3$};
	\node[state, right of=q2] (q4) {$q_4$};
	\node[state, accepting, right of=q3] (q5) {$q_5$};
	\node[state, below of=q5] (q6) {$q_6$};
	\draw
	(q1) edge[] node{b} (q3)
	(q3) edge[bend left] node{b} (q5)
	(q5) edge[bend left] node{b} (q3)
	
	(q5) edge[] node{a} (q6)

(q6) edge[bend left] node{a} (q4)
(q4) edge[bend left] node{a} (q6)
(q6) edge[bend left] node{b} (q2)

(q3) edge[] node{a} (q4)
(q4) edge[] node{b} (q2)

(q2) edge[] node{b} (q3)

	(q2) edge[bend left] node{a} (q1)
	(q1) edge[bend left] node{a} (q2)
      ;
  \end{tikzpicture}

Minimizes to:
\begin{table}[]
	\begin{tabular}{|c|c|c|c|c|c|c}
		\hline
		&  6 & 5 &  4 & 3  &   2 \\ \hline
		1 & X & X & X & X & $\checkmark$ \\ \hline
		2  &  X  & X & X & X \\ \hline
		3  & X & $\checkmark$ &  X\\ \hline
		4  &  $\checkmark$ & X \\ \hline
        5  & X \\ \hline
	\end{tabular}
\end{table}


\begin{tikzpicture}
	\node[state, initial] (q1) {$q_{1,2}$};
	\node[state, accepting, right of=q1] (q2) {$q_{3,5}$};
	\node[state, below right of=q1] (q3) {$q_{4,6}$};
	\draw
	(q1) edge[loop above] node{a} (q1)
	(q2) edge[loop above] node{b} (q2)
	(q3) edge[loop below] node{a} (q3)
	
	(q1) edge[] node{b} (q2)
	(q2) edge[] node{a} (q3)
	(q3) edge[] node{b} (q1)
	;
\end{tikzpicture}
\end{enumerate}

\newpage
\item \textbf{When dealing with strings, exponents denote repetition. For
  example, $a^2=aa$, $a^3=aaa$, $a^n= n a$'s.  Construct a DFA over
  $\{a,b\}$ that recognizes the language of all words ending in $a^xb^y$,
  where $x$ and $y$ are both odd. (Assume that $a^x$ includes the entire group
  of $a$'s.  For example, $aaaa$ can only be counted as $a^4$, and not as a
  single '$a$' followed by $a^3$). Minimize your DFA or ensure that it's
  already minimal.}
\\\\
  \begin{tikzpicture}
	\node[state, initial] (q1) {$q_1$};

	\node[state, above right of=q1] (q3) {$q_3$};
	\node[state, above of=q3] (q2) {$q_2$};

	\node[state, below right of=q1] (q4) {$q_4$};
	\node[state, below of=q4] (q5) {$q_5$};
	
	\node[state,  right of=q3] (q8) {$q_8$};
    \node[state, above of=q8] (q6) {$q_6$};

	\node[state, right of=q4] (q9) {$q_9$};
	\node[state, below of=q9] (q7) {$q_7$};

	\draw
	(q1) edge[bend left] node{a} (q3)
	(q1) edge[bend right] node{b} (q4)

	(q2) edge[bend right] node{a} (q3)
	(q3) edge[bend right] node{a} (q2)	
	
	(q3) edge[] node{b} (q8)	
	(q4) edge[] node{a} (q9)	
	
	(q4) edge[bend right] node{b} (q5)
	(q5) edge[bend right] node{b} (q4)	


	(q2) edge[above] node{b} (q8)
	(q6) edge[below] node{b} (q3)	


	(q6) edge[bend right] node{b} (q8)
	(q8) edge[bend right] node{b} (q6)	


	(q7) edge[above] node{a} (q4)
	(q5) edge[below] node{a} (q9)	

	(q9) edge[bend right] node{b} (q8)
	(q8) edge[bend right] node{a} (q9)	


	(q7) edge[bend right] node{a} (q9)
	(q9) edge[bend right] node{a} (q7)	
	
	;
\end{tikzpicture}

\newpage
\item \textbf{Write a regular expression that defines the language over
  $\{a,b,c\}$, consisting of all words that begin and end in a double ‘$c$’, contain at least one letter between the starting double ‘$c$’ and the ending double ‘$c$’, where $b$’s must occur in groups of 2  or more, and each group of $b$’s must be preceded by an ‘$a$’.}
\\\\
$$cc (a + abbb\kleene + c) (a + abbb\kleene + c)\kleene  cc$$
\newpage
\item \textbf{Write a regular expression that defines the language over
  ${a,b,c}$, consisting of all words in which $c$'s can only occur when an odd
  number of $a$'s have been seen up to that point. So $c$'s can occur between the 1st and 2nd $a$'s, and between the 3rd and 4th $a$'s, but not between the 2nd and 3rd $a$'s (and so on).}
\\\\

$$  b\kleene (a(aa)\kleene (b + c)\kleene )\kleene $$
\newpage
\item \textbf{Proofs: provide an argument for each answer. Make it as
  proof-like as you can.}
\begin{enumerate}
  \item For two regular languages $L1$ and $L2$, is $L1L2$ a proper subset of
    $L1$ always, sometimes, or never?
    \\\\
    $L1L2$ is never a proper subset of $L1$ since $L1L2$ is always as least as "big" as $L1$.   Unless
    $L2$ is $\emptyset$ . . ?  Is that possible?  If so, then $L1L2 = \{\}$, in which case, yes $L1L2 \subset L1$ 
  \item Prove or disprove (true or false): The regular expression $(a+b)\kleene c$
    describes the language consisting of all words which end in "$c$".
    \\\\
    False.  The word "$cc$ ends in $c$ but is \textbf{not} covered by this Regular Expression.
\end{enumerate}

\newpage
\item \textbf{Exercise 2.2.6a (p. 54) - Keep in mind that when reading binary
  from left to right, each additional bit multiplies the value of the
  previously seen bits by 2, and then may add 1 or not. Use this to your
  advantage, while keeping track of the number's value mod 5.}
\\\\\textbf{Give DFA's accepting the following languages over the alphabet $\{0,1\}$}
\\\\ The set of all strings beginning with a $1$ that, when interpreted as a binary integer, is a 
multiple of 5.  For example, strings $101$, $1010$, and $1111$ are in the language; $0$, $100$
and $111$ are not.
\\\\
I don't know!  I don't se any patterns here!!


\end{enumerate}

\input{footer.tex}
